...
public class menu_choix_personnage : MonoBehaviour 
{
	...
	{ 
		...
		listPlayerInfos = UnityEngine.Object.FindObjectsOfType<PlayerInfos> ();
	...
	void OnGUI()
	{
		...
		{	
			...
			else 
			{
				...
				GUI.Box (new Rect(10 + 3 * (Screen.width / 2 - Screen.width / 8) / 5, space * (i + 1) + backgroundHeight * i, (Screen.width / 2 - Screen.width / 8) / 5, backgroundHeight), listPlayerInfos[i - 1].teamNumber.ToString());
				if (listPlayerInfos[i - 1].ready)
				{
					GUI.Box (new Rect(10 + 4 * (Screen.width / 2 - Screen.width / 8) / 5, space * (i + 1) + backgroundHeight * i, (Screen.width / 2 - Screen.width / 8) / 5, backgroundHeight), "\u221A");
				}
